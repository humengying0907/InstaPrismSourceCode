...
# For users interested in detailed model convergence status, we've introduced the `get_loglikelihood_trace()` function to visualize how log likelihood of the model converges over iterations. Please note that the `get_loglikelihood_trace()` function requires significantly more time to execute as it computes log likelihood of the model at each iteration.
# example
# trace_obj = get_loglikelihood_trace(input_type = 'refPhi',bulk_Expr = bulk_Expr,refPhi = refPhi_obj,n.core = 16)
# plot(seq(1,trace_obj$n.iter),trace_obj$loglikelihood_trace,xlab = 'iterations',ylab = 'log likelihood')
...
get_loglikelihood_trace = function(input_type=c('raw','prism','refPhi','refPhi_cs'),
                                   ...
                                   refPhi=NULL,
                                   refPhi_cs=NULL,
                                   n.iter=NULL,n.core=1){
  trace = function(bulk_Expr,ref,n.iter,n.core){
    cms=intersect(rownames(bulk_Expr),rownames(ref))
    ref=ref[cms,]
    pboptions(type = "txt", style = 3, char = "=")
    res.list=pbapply(bulk_Expr[cms,,drop=F],2,function(x) bpFixedPointTrace(bulk=matrix(x),ref=ref,n_iter = n.iter),cl = n.core)
    return(res.list)
  }
